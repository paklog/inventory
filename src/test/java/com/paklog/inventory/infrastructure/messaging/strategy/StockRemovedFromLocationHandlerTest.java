...
class StockRemovedFromLocationHandlerTest {

    @Mock
    private InventoryCommandService inventoryCommandService;

    private ObjectMapper objectMapper;
    private StockRemovedFromLocationHandler handler;

    @BeforeEach
    void setUp() {
        objectMapper = new ObjectMapper();
        objectMapper.registerModule(new JavaTimeModule());
        handler = new StockRemovedFromLocationHandler(inventoryCommandService, objectMapper);
    }

    @Test
    void canHandle_correctEventType_returnsTrue() {
        assertTrue(handler.canHandle("paklog.inventory.stock-removed-from-location.v1"));
    }
    ...
    @Test
    void handle_callsDecreaseQuantityOnHand() throws Exception {
        String sku = "TEST-SKU-001";
        Location location = new Location("A1", "S1", "B1");
        int quantity = 50;
        StockRemovedFromLocation eventData = new StockRemovedFromLocation(sku, location, quantity);

        CloudEvent cloudEvent = CloudEventBuilder.v1()
                .withId("123")
                .withSource(URI.create("/test"))
                .withType("paklog.inventory.stock-removed-from-location.v1")
        ...
        verify(inventoryCommandService, times(1)).decreaseQuantityOnHand(sku, quantity);
    }
}
